...
import { SERVICES_DATA, CEO_DATA } from "../data";
...
export default function ServicesSection() {
  ...
  const getWhatsappUrl = (serviceName: string) => {
    ...
    const text = `Greetings Grand Internet Services. I would like to make an inquiry regarding the ${serviceName.trim()} service.`;
        ...
        <ScrollReveal delayMs={300} durationMs={1000}>
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3" id="services-grid">
            {SERVICES_DATA.map((service, idx) => {
              const wsUrl = getWhatsappUrl(service.name);
              return (
                <div
                  key={service.id}
                ...
                >
                  ...
                  <div className="mt-8 pt-5 border-t border-gray-100/50 flex items-center justify-between">
                    ...
                    <a
                      ...
                      className="flex h-7 w-7 items-center justify-center rounded-full bg-gray-50 border border-gray-100 group-hover:bg-[#990000] group-hover:text-white group-hover:border-transparent transition-all duration-300 transform group-hover:translate-x-1 cursor-pointer focus:outline-none focus:ring-2 focus:ring-[#990000]/50"
